...
public class Board {
	 ...
	 * Despues se verifica si el jugador actual gana y cambia el estado. Verifica si
	 * hay posibilidad de tirar y regresa el estado PLAYING. En caso de que nadie
	 * gane y que no halla espacios vacios regresa el estado DRAW.
	 */
	public State stepGame(Seed player, int selectedRow, int selectedCol) {
		...
		if (cells[selectedRow][0].content == player // 3 linea.
				&& cells[selectedRow][1].content == player && cells[selectedRow][2].content == player
				|| cells[0][selectedCol].content == player // 3 columna.
						&& cells[1][selectedCol].content == player && cells[2][selectedCol].content == player
				|| selectedRow == selectedCol // 3 en diagonal.
						&& cells[0][0].content == player && cells[1][1].content == player
						&& cells[2][2].content == player
				|| selectedRow + selectedCol == 2 // 3 en diagonal inversa.
						&& cells[0][2].content == player && cells[1][1].content == player
						&& cells[2][0].content == player) {

			return (player == Seed.CP1) ? State.CP1_WON : State.PLAYER_WON;
		} else {
			// Si nadie gana. Verifica si hay espacios en blanco y regresa PLAYING, de lo
			// contrario regresa DRAW.
			for (int row = 0; row < ROWS; ++row) {
				for (int col = 0; col < COLS; ++col) {
